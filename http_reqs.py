...
def find_journeys(trip:dict) -> dict:
    ...
    payload = {"p_auth": auth,
               ...
               "returnDate": "", "_returnDate": "", "locationMode": "1", "passengerType-1": trip["passengers"], "passengerType-4": "0",
               "passengerType-5": "0", "passengerType-2": "0", "passengerType-3": "0",
...
def parse_journey(journey: dict) -> dict:
    ...
    output_data = {
        ...
        "type": "bus" if "busCharacteristic" in journey else "train",  # optional (bus/train)
...
def find_journey(trip):
    ...
    src, dst, dep_date, pax = trip["src_id"], trip["dst_id"], trip["dates"], trip["passengers"]
    ...
    if journeys is None:
        return None
    try:
        cheapest_journey_loc = journeys["journeys"][0]# min(journeys["journeys"], key=lambda x: x.get("lowest_fare"))
